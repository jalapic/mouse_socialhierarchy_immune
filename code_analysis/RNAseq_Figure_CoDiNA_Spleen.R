...
DiffNet <- readRDS("results_RNAseqRDS/Spleen_final_DiffNet.RDS")
...
Graph = plot.CoDiNA.og(DiffNet, 
                    ...
                    path = glue("FinalVis_OG_Spleen.html"))
...
newGraph = plot.CoDiNA(DiffNet %>% 
                       ...
                       legend = F,
                       Cluster = F,
                       path = glue("filter_FinalVis_Spleen_topgene{my_top_selection}wTO.html"))
...
Graphx = plot.CoDiNA(DiffNet, 
                     ...
                     cutoff.internal = quantile(DiffNet$Score_internal, 0.25), 
                     layout = 'layout_nicely', 
                     path = glue("FinalVis_Spleen.html"))
...
rbind(nodes, links) %>%
  mutate(xx = factor( xx , levels = c("Nodes", "Links"))) %>%
  mutate(Var1 = factor(Var1, levels = c("a","g.ALPHA","g.SUB","U"))) %>%
  ggplot(aes(x = xx, y =  perc, fill = Var1))+
  geom_col(position = "stack",color ='white', alpha = 0.8)+
  scale_fill_manual(values = c("limegreen", "Purple4","Orange","grey70"),
  ...
  coord_flip() -> for_legend


png(filename = glue("results_figures/legend_CoDiNA_right.png"),
    width = 3.5, height = 3.5, units = "cm", res = 600)
